...
export default function ProofOfWorkTable() {
  const { state, createContractFromTask } = useMvp()
...
<div className='modal-actions'><button className='mbtn-ok' disabled={!exportPayload?.items.length} onClick={()=>{if(!exportPayload) return; const blob=new Blob([JSON.stringify(toJsonExport(exportPayload), null, 2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`${sel.taskId}.json`; a.click()}}>Export JSON</button><button className='mbtn-ok' disabled={!exportPayload?.items.length} onClick={()=>{if(!exportPayload) return; const blob=new Blob([toCsvExport(exportPayload)],{type:'text/csv'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`${sel.taskId}.csv`; a.click()}}>Export CSV</button><button className='mbtn-ok' onClick={()=>createContractFromTask(sel.taskId)}>Create Weekly Lead Contract</button></div></div></div>}</div>
}
