...
import { canUseFeature, getPaywallCopy, hasReachedLimit, type Plan } from '@/lib/billing/entitlements'
...
const defaultState = { agents: seedAgents, tasks: [], activityEvents: [], proofRecords: [], contracts: [], activeTaskId: null, selectedAgentId: null, toast: '', outreach: defaultOutreach, billing:{ plan:'free' as Plan, paywall:null as any }, usage:{ scansThisMonth:0, leadsRevealedThisMonth:0, draftsGeneratedThisMonth:0, exportsThisMonth:0, activeAgents:0, workspacesUsed:1, gmailConnected:false, inboxMonitoringEnabled:false } }
...
export function MvpProvider({ children }: { children: React.ReactNode }) {
  ...
  const api = useMemo(() => ({
    state,
    setPlan: (plan: Plan) => setState((s:any)=>({ ...s, billing:{ ...s.billing, plan } })),
    closePaywall: () => setState((s:any)=>({ ...s, billing:{ ...s.billing, paywall:null } })),
    requireFeature: (feature:any) => { const plan=state?.billing?.plan ?? 'free'; if (canUseFeature(plan, feature)) return true; setState((t:any)=>({ ...t, billing:{ ...t.billing, paywall:{ feature, ...getPaywallCopy(feature, plan), currentPlan: plan } } })); return false },
    ...
    addAgent: (agent: Agent) => setState((s:any)=>{ const plan=s.billing?.plan ?? 'free'; if (hasReachedLimit(plan, 'maxActiveAgents', s.usage?.activeAgents ?? 0)) return { ...s, billing:{ ...s.billing, paywall:{ feature:'activeAgents', ...getPaywallCopy('recurringAgents', plan) } } }; return { ...s, agents:[agent, ...s.agents], usage:{ ...s.usage, activeAgents:(s.usage?.activeAgents ?? 0)+1 }, activityEvents:[{ id: crypto.randomUUID(), type:'AGENT_SPAWNED', title:'Agent spawned', description:`${agent.name} is now available.`, timestamp:new Date().toISOString(), status:'success', agentName:agent.name }, ...s.activityEvents], toast:'Agent spawned' }}),
    ...
    connectGmail: () => setState((s:any)=>{ const plan=s.billing?.plan ?? 'free'; if(!canUseFeature(plan,'connectGmail')) return { ...s, billing:{ ...s.billing, paywall:{ feature:'connectGmail', ...getPaywallCopy('connectGmail', plan) } } }; return { ...s, outreach:{ ...s.outreach, gmailConnected:true, mode:'live' }, usage:{ ...s.usage, gmailConnected:true }, activityEvents:[{ id: crypto.randomUUID(), type:'GMAIL_CONNECTED', title:'Gmail connected', description:'OAuth scopes approved for outreach inbox.', timestamp:new Date().toISOString(), status:'success' }, ...s.activityEvents], toast:'Gmail linked' }}),
